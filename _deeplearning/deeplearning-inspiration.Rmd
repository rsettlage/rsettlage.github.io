...
This starts the same way: get some data.  Let's simulate it again, in a future post, we will go through a more expansive and real dataset.  For now, let's imagine a scenario where we are interested in responder/non-responder vs drug dose.  We will imagine that at some dose level, people transition between responder/non-responder with some high probability.
